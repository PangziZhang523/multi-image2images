...
# End ORig: 
# Moha: -------------------------
Examples = collections.namedtuple("Examples", "paths, inputs, targets_1,targets_2,targets_3, count, steps_per_epoch")

Model = collections.namedtuple("Model", "outputs_1, outputs_2, outputs_3, predict_real, predict_fake, discrim_loss, discrim_grads_and_vars, gen_loss_GAN, gen_loss_L1, gen_grads_and_vars, train,gen_loss,discrim_loss_fake,discrim_loss_real,learning_rate_g,learning_rate_d") #, gen_loss_dice, gen_loss_jaccard, gen_loss_Tversky")
# end MOha
...
def create_model_MT(inputs, targets_1, targets_2, targets_3):
    ...
    targets=tf.concat([targets_1, targets_2, targets_3],axis=3)

    global_step = tf.train.get_or_create_global_step()
    incr_global_step = tf.assign(global_step, global_step+1)

    with tf.variable_scope("learning_rate_g"):
        learning_rate_g = tf.train.exponential_decay(a.g_lr,global_step=global_step,decay_steps=1000,decay_rate=0.8,staircase=False)

    with tf.variable_scope("learning_rate_d"):
        learning_rate_d = tf.train.exponential_decay(a.d_lr,global_step=global_step,decay_steps=1000,decay_rate=0.8,staircase=False)
    ...
    with tf.name_scope("discriminator_train"):
        ...
        discrim_optim = tf.train.AdamOptimizer(learning_rate_d, a.beta1)
    ...
    with tf.name_scope("generator_train"):
        with tf.control_dependencies([discrim_train]):
            ...
            gen_optim = tf.train.AdamOptimizer(learning_rate_g, a.beta1)
    ...
    update_losses = ema.apply([discrim_loss, gen_loss_GAN, gen_loss_L1,
                               gen_loss,discrim_loss_real,discrim_loss_fake]) #, gen_loss_jaccard, gen_loss_dice,gen_loss_Tversky])

    outputs_1=outputs[:,:,:,:3]
    outputs_2=outputs[:,:,:,3:6]
    outputs_3=outputs[:,:,:,6:]
...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        ...
        b_images = preprocess(raw_input[:,width//4:2*width//4,:])
        c_images = preprocess(raw_input[:,2*width//4:3*width//4,:])
        d_images = preprocess(raw_input[:,3*width//4:,:])
        

    if a.which_direction == "AtoB":
        inputs, targets_1, targets_2, targets_3 = [a_images, b_images, c_images, d_images]
        #targets_2=c_images
    ...
    with tf.name_scope("target_images"):
        ...
        target_3_images = transform(targets_3)
#        target_images=tf.concat([target_1_images,target_2_images],axis=3)
    ...
    paths_batch, inputs_batch, targets_1_batch, targets_2_batch, targets_3_batch = tf.train.batch([paths, input_images, target_1_images, target_2_images, target_3_images], batch_size=a.batch_size)
...
def save_images(fetches, step=None):
    ...
    for i, in_path in enumerate(fetches["paths"]):
        ...
        for kind in ["inputs", "outputs_1","targets_1", "outputs_2","targets_2", "outputs_3", "targets_3"]:
...
def main():
    ...
    model = create_model_MT(examples.inputs, examples.targets_1, examples.targets_2,examples.targets_3)


    inputs = deprocess(examples.inputs)
    targets_1 = deprocess(examples.targets_1)
    targets_2 = deprocess(examples.targets_2)
    targets_3 = deprocess(examples.targets_3)
    outputs_1 = deprocess(model.outputs_1)
    outputs_2 = deprocess(model.outputs_2)
    outputs_3 = deprocess(model.outputs_3)
    ...
    with tf.name_scope("encode_images"):
        display_fetches = {
            ...
            "targets_3": tf.map_fn(tf.image.encode_png, converted_targets_3, dtype=tf.string, name="target_3_pngs"),
            ...
            "outputs_3": tf.map_fn(tf.image.encode_png, converted_outputs_3, dtype=tf.string, name="output_3_pngs"),
    ...
    tf.summary.scalar("learning_rate_g", model.learning_rate_g)
    tf.summary.scalar("learning_rate_d", model.learning_rate_d)
        ...
        if a.mode == "test":
            ...
            for step in range(max_steps):
                ...
                if should(a.progress_freq):
                    ...
                    fetches["learning_rate_d"] = model.learning_rate_d
                    fetches["learning_rate_g"] = model.learning_rate_g
                ...
                if should(a.progress_freq):
                    ...
                    print("learning_rate_g -----------", results["learning_rate_g"])
                    print("learning_rate_d -----------", results["learning_rate_d"])
...
#            Image_combined[:,CROP_SIZE:,:]=Image_t2
        ...
        if task_No==3:        
            ...
            Image_t3=io.imread(a.input_dir_all+'/Targets_3'+'/'+ filename)

            # combinedimage 2048*512
            Image_combined=np.zeros(shape=[Image.shape[0],4*Image.shape[1],3], dtype='uint8')
            Image_combined[:,:CROP_SIZE,:]=Image
            Image_combined[:,CROP_SIZE:2*CROP_SIZE,:]=Image_t1            
            Image_combined[:,2*CROP_SIZE:3*CROP_SIZE,:]=Image_t2  
            Image_combined[:,3*CROP_SIZE:,:]=Image_t3    
...
for cv in range(1,len(CvDirs)+1): # #################
    ...
    try:
        ...
        print('a.output_dir没删除')
    except:
        no=1
    main()
    try:
        # shutil.rmtree(a.input_dir)
        print('a.input_dir没删除')
    except:
        no=1
